...
bool TMP1075_installed(int device_index)
{
    ...
    ESP_LOGI(TAG, "Reading configuration register - Fan %d", device_index);
    ESP_ERROR_CHECK(register_read(TMP1075_I2CADDR_DEFAULT + device_index, TMP1075_CONFIG_REG, data, 2));
    //ESP_LOGI(TAG, "Configuration[fan %d] = %02X %02X", device_index, data[0], data[1]);
...
uint8_t TMP1075_read_temperature(int device_index)
{
    ...
    ESP_LOGI(TAG, "Temperature[sensor %d] = %d", device_index, data[0]);
    return data[0];
}
